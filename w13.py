...
# Base URL to scrape
base_url = 'https://bitmars.io/asic-miners/page/'
total_pages = 6  # Specify the total number of pages to scrape
...
for page_number in range(1, total_pages + 1):
    ...
    product_elements = soup.find('ul', class_='products').find_all('li', class_='product')
    ...
    for index, product in enumerate(product_elements, start=(page_number - 1) * len(product_elements) + 1):
        ...
        # Extract product URL
        product_url_tag = product.find('a', class_='woocommerce-LoopProduct-link')
        ...
        product_name_tag = product.find('h4', class_='woocommerce-loop-product__title')
